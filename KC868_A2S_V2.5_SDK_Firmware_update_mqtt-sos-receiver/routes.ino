...
void routes() {
  ...
  server.on("/communication", HTTP_GET, handleCommunication);
  server.on("/relays", HTTP_GET, handleRelays);

  server.on("/devicesos", HTTP_GET, handlesos);


  // server.on("/form2", HTTP_GET, handleForm2);
  server.on("/submit-form1", HTTP_POST, handleForm1Submit);
  server.on("/submit_communication", HTTP_POST, handleSubmitCommunication);

  // server.on("/submit-form2", HTTP_POST, handleForm2Submit);
  server.on("/styles.css", HTTP_GET, handleCSS);
  server.on("/script.js", HTTP_GET, handleJS);
  server.on("/header.js", HTTP_GET, handleHeaderJS);
...
void handleRelays() {
  ...
  String form = readFile("/relaysettings.html");

  String html;
  html.reserve(header.length() + form.length());
  html = header + form;
  html = replaceHeaderContent(html);
  Serial.println("relays Available");
  server.send(200, "text/html", html);
  delay(1000);
...
void handlesos() {
  ...
  String header = readFile("/header.html");
  String form = readFile("/device_sos_list.html");
...
void handleCommunication() {
  ...
  String header = readFile("/header.html");
  String form = readFile("/communication.html");
...
void handleSubmitCommunication() {
  loadingConfigFile = true;
  if (!isAuthenticated()) {
    server.sendHeader("Location", "/");
    server.send(302);
  ...
  doc["socket_communication"] = server.hasArg("socket_communication");
  doc["server_ip"] = server.arg("server_ip");
  doc["server_port"] = server.arg("server_port");


  doc["http_communication"] = server.hasArg("http_communication");
  ...
  server.sendHeader("Location", "/communication");
  server.send(302);
...
void handleForm1Submit() {
  ...
  if (server.hasArg("http_communication"))
    doc["http_communication"] = server.arg("http_communication");
...
void handleRestartDevice() {
  ...
  String html = "<html><head>"
                ...
                "<a href='http://"
                + ipAddress + "'>Go to Device Page IP: " + ipAddress + "</a>"
                                                                       "<script>"
                                                                       "setTimeout(function(){"
                                                                       "location.href='http://"
                + ipAddress + "';"
...
void loadJS(String filename) {
  String js = readFile(filename);
  server.send(200, "text/javascript", js);  // Use correct MIME typed:\Aurdino Examples\KS868_A16M_V1_1_SDK_Firmware_update_mqtt\data\script.js
}
...
void handleHeaderJS() {
  String js = readFile("/header.js");
  server.send(200, "text/javascript", js);  // Use correct MIME typed:\Aurdino Examples\KS868_A16M_V1_1_SDK_Firmware_update_mqtt\data\script.js
}
void handlescriptcommunicationJS() {
  String js = readFile("/scriptcommunication.js");
  server.send(200, "text/javascript", js);  // Use correct MIME type
}
...
void sendNotificationToServerHttp(String jsonData) {

  {

    Serial.println("Sending to HTTP: " + jsonData);
