...
#define RELAY_GATE0 2   // KC868-A2 v2.5: Relay1 D5  //Gate1
#define RELAY_GATE1 15  // KC868-A2 v2.5: Relay2 D6 //Gate1


#define GATE1 15  // KC868-A2 v2.5: Relay1 // GATE1
#define GATE2 2   // KC868-A2 v2.5: Relay2 // GATE2
...
void checkAllDI() {
  ...
  for (int i = 0; i < DI_COUNT; i++) {
    ...
    // }
    int reset_settings_duration = config["reset_settings_duration"];
    ...
    if (i == DI_FACTORY_RESET) {
      ...
      if (currentState) {
        ...
        } else if (millis() - factoryResetStartTime >= 1000 * reset_settings_duration && millis() - factoryResetStartTime <= 1000 * (reset_settings_duration + 10)) {
          restoreDefaultConfig();
...
void digitalLoop() {
  ...
  //   //Serial.println(pcf8574_DI.digitalRead(i));
  //   if (pcf8574_DI.digitalRead(i)) {
  //     Serial.println("-OFF");
  //     // pcf8574_RE1.digitalWrite(i, LOW);
  //   } else {
  //     Serial.println("-ON");
...
void updateRelayStatusAction(int relayNum, bool status) {
  ...
  if (relayNum >= 0) {
    ...
    digitalWrite(relayNum, status ? HIGH : LOW);
...
void updateRelayStatus(int relayNum) {
   ...
   Serial.println(String(relayNum) + ": Relay Number ");

  if (relayNum >= 0) {
    relayStates[relayNum] = !relayStates[relayNum];
    digitalWrite(relayNum, relayStates[relayNum] ? LOW : HIGH);

     if (relayNum == 0) updateRelayStatusAction(RELAY_GATE0, relayStates[relayNum] ? LOW : HIGH);
          if (relayNum == 1) updateRelayStatusAction(RELAY_GATE1, relayStates[relayNum] ? LOW : HIGH);
  ...
  {
    Serial.println(String(relayNum) + ": Relay Number is Invalid");
  }
}
...
void relaysSetup() {
  ...
  pinMode(RELAY_GATE0, OUTPUT);
  pinMode(RELAY_GATE1, OUTPUT);
  ...
  digitalWrite(RELAY_GATE1, LOW);
  {
    ...
    updateJsonConfig("config.json", "relay0", "false");
    updateJsonConfig("config.json", "relay1", "false");
