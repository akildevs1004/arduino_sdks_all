...
document.addEventListener("DOMContentLoaded", function () {
  ...
  selectElement = document.getElementById("reset_settings_duration");
  ...
  /*try {
    selectElement = document.getElementById("temperature_read_interval");
    options = generateOptions3();
...
function generateOptions3() {
  const options = [];

  // Generate options for seconds (5s to 55s)
  for (let sec = 10; sec <= 120; sec += 5) {
...
function loadConfigFileData() {
  ...
  try {
    ...
    //   ? jsonData.wifi_gateway
    //   : jsonData.wifi_gateway == "{wifi_gateway}"
    //   ? ""
    //   : jsonData.wifi_gateway == "undefined"
    //   ? ""
    //   : jsonData.wifi_gateway;
    // document.getElementById("wifi_subnet").value = jsonData.wifi_subnet
    //   ? jsonData.wifi_subnet
    //   : jsonData.wifi_subnet == "{wifi_subnet}"
    //   ? ""
    //   : jsonData.wifi_subnet == "undefined"
    //   ? ""
    //   : jsonData.wifi_subnet;

    const gw = jsonData.wifi_gateway;
    document.getElementById("wifi_gateway").value =
      gw && gw !== "{wifi_gateway}" && gw !== "undefined" ? gw : "";

    const subnet = jsonData.wifi_subnet;
    document.getElementById("wifi_subnet").value =
      subnet && subnet !== "{wifi_subnet}" && subnet !== "undefined"
        ? subnet
        : "";
    ...
    document.getElementById("reset_settings_duration").value =
      jsonData.heartbeat == "{reset_settings_duration}"
        ? ""
        : jsonData.reset_settings_duration;
    ...
    document.getElementById("temperature_read_interval").value =
      jsonData.temperature_read_interval == "{temperature_read_interval}"
        ? ""
    ...
    console.log(jsonData.temp_checkbox);
...
function scanWiFi() {
  ...
  fetch("/wifi-scan")
    ...
    .then((data) => {
      ...
      document.getElementById("wifiloading").innerHTML =
        "<span style=color:green>wifi scanning is completed</span>";
    })
